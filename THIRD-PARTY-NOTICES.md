...
resolved by `Cargo.lock`, `caddy/go.mod`, `caddy/go.sum`, and
`frontend/pnpm-lock.yaml`. Those dependencies remain under their respective
licenses. The reviewed dependency set uses these SPDX license families and
expressions:
...
The container build also incorporates the official Debian, Rust, Node.js, and
Go base images named in `Dockerfile`. Their operating-system packages and
runtime components retain their upstream licenses.
